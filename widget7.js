(function () {
  ...
  // EXPERIMENTAL FORK of widget6.js - "Widget 7" in the A/B comparison on
  // /demo/introlinq. Same idea as Widget 6 (append something on a
  // confirmed no-match page instead of showing nothing) but swaps the
  // plain text-line fallback for an actual mini-carousel of real experts
  // (dynamically loads the existing carousel.js, the same widget already
  // used elsewhere on the site) - real named people with photos generally
  // outperform generic text, per everything else this session found.
  // NOT deployed anywhere live yet, and deliberately not silent about it
  // when it does go live - showing a visible carousel of experts on pages
  // unrelated to their content is a bigger change to what a publisher's
  // page displays than the Widget 6 text line was, and should be a
  // publisher-notified decision, not a silent one. Search this file for
  // "NO-MATCH FALLBACK" for what's different from Widget 6. Own guard
  // flag/script-selector so this can run alongside the other widget
  // variants without colliding.
  var API = 'https://www.introlinq.com/api/match';
  var script = document.currentScript || document.querySelector('script[src*="widget7.js"]');
  var PUB = (script && (script.getAttribute('data-publisher') || script.getAttribute('data-site'))) || window.IL_PUBLISHER_ID || null;
  if (!PUB) return;
  if (window.__ilWidget7Init) return;
  window.__ilWidget7Init = true;
    ...
    function postScan() {
      ...
      attempt()
        ...
        .then(function (data) {
          ...
          // NO-MATCH FALLBACK (Widget 7 only): the page was genuinely
          // scanned and confirmed to have nothing relevant - handleMultiVariant
          // sets noMatch:true specifically for this case (see api/match.js),
          // distinct from "not scanned yet" (which has no `cached` field at
  ...
  function injectStyles(cfg) {
    ...
    s.textContent =
      ...
      // NO-MATCH FALLBACK (Widget 7 only - see this file's header comment
      // for what's different from Widget 6). Just a thin top border and
      // spacing to separate it from the article - carousel.js brings its
      // own scoped internal styling (header, cards, arrows), so there's
      // nothing else to style here.
      '#il7-fallback{margin-top:1.75rem;padding-top:1.25rem;border-top:1px solid rgba(26,26,46,0.1)}';
    document.head.appendChild(s);
  }

  // NO-MATCH FALLBACK (Widget 7 only). Appended once per page, at the end
  ...
  // hover-popup mechanic at all. Unlike Widget 6's plain text line, this
  // dynamically loads the real carousel.js (the same widget already used
  // elsewhere on the site, not a reimplementation) - it has no singleton
  // guard by design (see its own header comment), so it's always safe to
  // inject even if this publisher separately has a real carousel
