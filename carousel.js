(function () {
  ...
  style.textContent = [
    ...
    '#'+uid+' .ilc-arrow:disabled{opacity:0.3;cursor:default;pointer-events:none}',
    ...
    '#'+uid+' .ilc-track{display:flex;gap:0.875rem;overflow-x:auto;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;scrollbar-width:none;padding-bottom:2px}',
    '#'+uid+' .ilc-track::-webkit-scrollbar{display:none}',
    '#'+uid+' .ilc-card{flex:0 0 160px;scroll-snap-align:start;border:1.5px solid #e4e4ee;border-radius:14px;padding:1rem 0.875rem;background:#fff;display:flex;flex-direction:column;align-items:center;text-align:center;gap:0.5rem;transition:box-shadow .15s,transform .15s;cursor:default}',
  ...
  function render(data) {
    ...
    container.innerHTML = '<div class="ilc-wrap">'
      ...
      +'<div class="ilc-label">Book an expert</div>'
      +'<div class="ilc-arrows">'
      +'<button class="ilc-arrow" id="'+uid+'-prev" onclick="document.getElementById(\''+uid+'-track\').scrollBy({left:-360,behavior:\'smooth\'})" aria-label="Previous">'
      ...
      +'</button>'
      +'<button class="ilc-arrow" id="'+uid+'-next" onclick="document.getElementById(\''+uid+'-track\').scrollBy({left:360,behavior:\'smooth\'})" aria-label="Next">'
