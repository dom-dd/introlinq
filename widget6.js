(function () {
  ...
  // EXPERIMENTAL FORK of widget5.js (not widget.js directly) - "Widget 6"
  ...
  // pushes the article's own content around. Deliberately NOT a permanent
  // directory box on ~81% of a publisher's pages they never explicitly
  // agreed to. Every real match still renders exactly like Widget 5 - this
  // only fires on the "currently shows nothing" case. Deliberately forked
  // from widget5.js rather than widget.js so the diff between the two
  // stays small and this can be folded directly into Widget 5 later with
  // minimal work if it proves out - search this file for "NO-MATCH
  // FALLBACK" to find every piece that would need to move. Own guard
  ...
  var script = document.currentScript || document.querySelector('script[src*="widget6.js"]');
  var PUB = (script && (script.getAttribute('data-publisher') || script.getAttribute('data-site'))) || window.IL_PUBLISHER_ID || null;
  if (!PUB) return;
  if (window.__ilWidget6Init) return;
  window.__ilWidget6Init = true;
    ...
    function postScan() {
      ...
      attempt()
        ...
        .then(function (data) {
          ...
          // NO-MATCH FALLBACK (Widget 6 only): the page was genuinely
  ...
  function injectStyles(cfg) {
    ...
    // 'underline' uses real text-decoration, not border-bottom -
    // border-bottom draws at the bottom edge of the inline box, which font
    // line-box metrics push well below the actual glyph descenders no
    // matter the line-height (tried pinning line-height:1 first; still sat
    // too low - the gap is baked into the font's own metrics, not the
    // surrounding paragraph). text-decoration is a native underline - the
    // same mechanism a plain hyperlink uses - so it hugs the text properly
    // by default; text-underline-offset nudges it a couple px for breathing
    // room without floating away from the words. 'fill' keeps border-bottom
    // on purpose: it's the bottom edge of a tinted highlighter-block look,
    // not meant to hug text tightly the way a plain underline should, and
    // line-height:1 keeps that block snug to its own line rather than the
    // host paragraph's taller one.
    ...
    s.textContent =
      ...
      // #il6-hookwrap (headline + CTA pill) is the only thing on the page -
