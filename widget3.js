(function () {
  ...
  // EXPERIMENTAL FORK of widget.js - "Widget 3" in the A/B comparison on
  // /demo/introlinq. Shows NO expert identity pre-click at all: a punchy
  // outcome-focused hook + one CTA, revealing who to actually talk to only
  // after a click through to the expert board. Own guard flag/script-
  // selector so this can run alongside the other widget variants without
  // colliding - see widget.js's own comment on why the guard exists at all.
  // Only ever loaded manually for this comparison, never part of the
  // normal install flow.
  var API = 'https://www.introlinq.com/api/match';
  var script = document.currentScript || document.querySelector('script[src*="widget3.js"]');
  var PUB = (script && (script.getAttribute('data-publisher') || script.getAttribute('data-site'))) || window.IL_PUBLISHER_ID || null;
  if (!PUB) return;
  if (window.__ilWidget3Init) return;
  window.__ilWidget3Init = true;
